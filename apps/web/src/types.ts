...
export type EndpointType = "chat" | "image_generation" | "video_generation_async";
...
export type ApiTemplate = {
  ...
  name: string;
  category: ModelCategory;
  protocol: "openai_chat" | "gemini_native" | "claude_native" | "image_generation" | "video_async" | "other" | (string & {});
  method: "GET" | "POST";
  path: string;
  is_async: boolean;
...
export type ParamSchemaItem = {
  key: string;
  label: string;
  type: "input" | "textarea" | "number" | "select" | "boolean";
  required?: boolean;
  default?: unknown;
  min?: number;
  max?: number;
...
export type ValidateBaseUrlResponse = {
  ok: boolean;
  origin?: string;
  message: string;
  debug?: unknown;
